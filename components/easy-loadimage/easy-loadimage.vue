<template>
	<view class="easy-loadimage" :id="uid">
		<image class="origin-img" :src="imageSrc" :mode="mode" v-if="loadImg&&!isLoadError" v-show="showImg" :class="{'no-transition':!openTransition,'show-transition':showTransition&&openTransition}"
		 @load="handleImgLoad" @error="handleImgError" :style="originImg">
		</image>
		<!-- <image class="origin-img" :src="imageSrc" :mode="mode" v-if="loadImg&&!isLoadError" v-show="showImg" :class="{'no-transition':!openTransition,'show-transition':showTransition&&openTransition}"
		...
		methods: {
			...
			onScroll(scrollTop) {
				...
				query.select('#' + id).boundingClientRect(data => {
					// console.log('我组件滚轮', data.top, this.viewHeight, data.top - this.viewHeight < 0)
					if (!data) return;
					// if (data.top - this.viewHeight < 80) {
					if (data.top - this.viewHeight < 300) {
					// if (data.top - this.viewHeight < this.viewHeight ) {
						this.loadImg = true;
					}
				}).exec()
			},

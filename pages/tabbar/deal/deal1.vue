	...
	<view class="main">
				...
				<scroll-view class="mainList" scroll-y="true" @scrolltolower="toBottom" @scroll='myScroll' @scrolltoupper='scrolltoupper' :lower-threshold="100" :refresher-triggered="refreshState"
				 :refresher-enabled="scroll_refresher_enabled"  @refresherrefresh="pullRefresh" v-if="item.list.length>0">
					...
					<view class="mainList-item" :class="[item3.active ? '' : 'my-bg-gary']" v-for="(item3,index3) in item.list" :key="index3">
						<deal-item :dealItem="item3" :scrollTop='scrollTop'></deal-item>
					</view>
		...
		<view class="my-bottom fs-18">
			...
			<view class="bottom-right">
				{{ surplusUsdt | four }} <text class="unit">USDT</text>
			</view>
		</view>
		<!-- 确认弹窗 -->
		...
		methods: {
			...
			async dangeUpdateStrategyTradingop(item,index) {
				...
				if(this.clist[this.currentIndex].list[index].isGiveUp == true) {
					...
					console.log('选择交易了')
					// 执行策略循环
					if(this.clist[this.currentIndex].list[index].Type == null || this.clist[this.currentIndex].list[index].Type == 0 || this.clist[this.currentIndex].list[index].Type == 4 ) {
						myType = 1
					}else {
			...
			openSocket() {
				...
				try {
					...
					this.socketTask = uni.connectSocket({
						...
						success: (data) => {
							console.log("交易页面websocket创建连接成功");
						},
						fail: () => {
							console.log("交易页面websocket创建连接失败");
							
						}
					})
				} catch (e) {
					//TODO handle the exception
					console.log('交易页面连接封装的出错了')
				}
			
				// uni.onSocketOpen((ress) => {
				this.socketTask.onOpen((ress) => {
					console.log('交易页面websocket成功连接', ress)
					// 发送消息
					clearInterval(this.timer2)
					this.timer2 = null
				...
				this.socketTask.onError((err) => {
					console.log('交易页面websocket失败连接', err)
					clearInterval(this.timer2)
					this.timer2 = null
					// toast('系统错误，请重新尝试')
					// this.openSocket()
				...
				this.socketTask.onClose((res) => {
					clearInterval(this.timer2)
					this.timer2 = null
					console.log('交易页面websocketWebSocket 已关闭！');
					// 是已经关闭了
					// uni.closeSocket()
					this.openSocket()
				...
				this.socketTask.onMessage((ress) => {
					...
					this.clist[this.currentIndex].list.map((item,index)=>{
						if(item.Symbol == 'Symbol') {
							// this.clist[this.currentIndex].list
							let Symbol = item.EtcUsdt
							let EtcAmount = item.EtcUsdt
							let EtcUsdt = item.EtcUsdt
							let newprice = item.EtcUsdt
							let EtcProfit = item.EtcProfit
							//计算盈利百分比
							this.GetEtcPercent(EtcUsdt, EtcProfit)
							//预估盈利
							this.GetProfit(Symbol, EtcAmount, EtcUsdt, newprice)
						}
					})
					...
					if (Object.prototype.toString.call(ress.data) === '[object ArrayBuffer]') {
						...
						// let myDate = new Date();
						// let mytime = myDate.toLocaleTimeString();
						// console.log('交易页面websocketWebSocket 收到了',mytime, info)
						...
						if (info.ch) {
							...
							// console.log('交易页面websocket收到了',temp5)
							...
							if (temp5.ch.match(/market\.(\S*)\.kline.1day/) != null) {
								...
								if (temp5.ch.match(/market\.(\S*)\.bbo/) != null) {
									// 如果是中间的
